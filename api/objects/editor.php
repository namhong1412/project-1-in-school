<?php
	...
	Class Editor extends Database {
		...
		function delete() {
			// query
			$query = "UPDATE `admin` SET `kinds`= ".$this->kinds." WHERE `admin_id` = ".$this->admin_id;
			return mysqli_query($this->get_connection(), $query);
		}

		function read() {
			// query 
			$query = "SELECT * FROM `admin` WHERE `kinds` = 1 OR `kinds` = 2";
		...
		function check_exist() {
			// query 
			$query = "SELECT `kinds` FROM `admin` WHERE `kinds` = 2 AND `admin_id` = ".$this->admin_id;
			$query_statement = mysqli_query($this->get_connection(), $query);
			$check = mysqli_num_rows($query_statement);
			return $check;
		}
